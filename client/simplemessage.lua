...
function message.unpack(data)
	...
	local _, dot_pos = string.find(msg_head.msgname, '%.')
	if dot_pos ~= nil then
		msg_head.msgname = string.sub(msg_head.msgname, dot_pos + 1, string.len(msg_head.msgname))
		utils.print("string.sub... msgname=" .. msg_head.msgname .. " dot=" .. dot_pos)
	end
